...
const targetPosition = { x: 0.35, y: 0.15, z: 0.1 };
...
window.addEventListener('keydown', (event) => {
  if (event.key === 'p') {
    ...
    gsap.to(camera.position, {
      ...
      ease: 'power3.inOut',
      onUpdate: () => {
        // Ensure the camera keeps looking at the center (0, 0, 0)
        camera.lookAt(0, 0, 0);
      },
    });
  }
...
// Adjust zoom functionality to modify the camera's FOV
window.addEventListener('wheel', (event) => {
  const zoomSpeed = 1; // Adjust zoom speed
  if (event.deltaY < 0) {
    // Zoom in
    camera.fov = Math.max(20, camera.fov - zoomSpeed); // Clamp minimum FOV to 20
  } else {
    // Zoom out
    camera.fov = Math.min(75, camera.fov + zoomSpeed); // Clamp maximum FOV to 75
  }
